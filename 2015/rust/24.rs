...
fn solve(n: &Vec<u64>, target: u64) -> u64 {
    ...
    for s in 1..n.len() {
        for comb in combinations(s, n) {
            if comb.iter().sum::<u64>() != target { continue; }

            if quant.len() == 0 || comb.len() < quant.len() || comb.len() == quant.len() 
...
fn main() {
    let inp = std::fs::read_to_string("../input/24").unwrap().lines().map(|n| n.parse::<u64>().unwrap()).collect::<Vec<u64>>();
