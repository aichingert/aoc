...
fn solve(hap: &HashMap<(String, String), i32>, ppl: &HashSet<String>) -> i32 {
    ...
    let perms = permutations(
        ppl.len(),
        &mut ppl.iter().map(|p| p.clone()).collect::<Vec<String>>(),
    );
    ...
    for perm in perms {
        ...
        ans = ans.max(
            cur + hap[&(perm[0].clone(), perm[ppl.len() - 1].clone())]
                + hap[&(perm[ppl.len() - 1].clone(), perm[0].clone())],
        )
    }
...
fn parse() -> (HashMap<(String, String), i32>, HashSet<String>) {
    ...
    for line in std::fs::read_to_string("../input/13").unwrap().lines() {
        ...
        happiness.insert(
            (vls[0].to_string(), vls[10][..vls[10].len() - 1].to_string()),
            hap,
        );
