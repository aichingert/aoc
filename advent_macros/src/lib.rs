...
pub fn add_modules(item: TokenStream) -> TokenStream {
    ...
    imports::add_days_of_year(&tokens[..])
        .join("\n")
        .parse()
        .unwrap()
}
...
pub fn add_fn_pointers(_item: TokenStream) -> TokenStream {
    ...
    lines.push(String::from(
        "fn no_solution() { println!(\"Not solved yet\"); }",
    ));
    lines.push(format!(
        "const FN_POINTER: [[fn() -> (); 25]; {}] = [",
        cur_year - 2015 + 1
    ));

    lines.push(
        (2015..=cur_year)
            .map(functions::add_fns_of_year)
            .collect::<Vec<_>>()
            .join(","),
    );
    lines.push(String::from("];"));
...
pub fn aoc_main(_item: TokenStream) -> TokenStream {
    "pub fn solve() { println!(\"ADVENT OF CODE\"); }"
        .parse()
        .unwrap()
}
